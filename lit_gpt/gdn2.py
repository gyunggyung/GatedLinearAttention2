...
class GatedDeltaNet2(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.use_kaczmarz_step = use_kaczmarz_step
        self.kaczmarz_eps = kaczmarz_eps
        ...
        if use_kaczmarz_step:
            self.eta_proj = nn.Linear(hidden_size, self.num_heads, bias=True)
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if self.use_kaczmarz_step:
            # KLA-style dynamic step size. This preserves the GDN-2 kernel
            # interface by folding the scalar step into the erase/write gates.
            k_heads = rearrange(k.float(), "... (h d) -> ... h d", d=self.head_k_dim)
            k_norm_sq = k_heads.square().sum(dim=-1, keepdim=True).clamp_min(self.kaczmarz_eps)
            eta = rearrange(self.eta_proj(hidden_states).sigmoid(), "... h -> ... h 1")
